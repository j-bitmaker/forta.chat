...
- `docs/manual-verification.md` — все починки, которым нужна проверка на аппарате. Раздел «Ожидают проверки» (16
...
1. **#809 п. 2 — Forta звонит, пока человек уже говорит в Bastyon.** Владелец выбрал вариант А (2026-09-18), сделано:
   ...
   (`sahere1`), ответ в Bastyon — тоже (`ownerb3`: рингер Forta снят через 28 мс после пуша); остался взгляд на счётчик
   в списке чатов — запись «Forta перестаёт звонить, когда
