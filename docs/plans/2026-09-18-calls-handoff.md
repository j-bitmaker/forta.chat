...
15. **iPhone** — 2026-09-23 подключён iPhone XR (iOS 17.3.1, Developer Mode по данным `devicectl` выключен). Два трека:
    - трек А, Safari: оба отчёта (#538 iOS 18.7 «нет в списке приложений/микрофон», #1276 iPhone 7 Plus iOS 15.8.8
      «не слышу») — `Platform: web`, проверяются в Safari без сборки;
