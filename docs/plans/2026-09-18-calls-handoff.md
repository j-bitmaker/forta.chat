...
- `docs/manual-verification.md` — все починки, которым нужна проверка на аппарате. Раздел «Ожидают проверки» (8
...
11. **E2E** — переменная задана владельцем 2026-09-22; после починки автозамены 3/4 сценария зелёные, «Send a text
    message» переписан и ждёт прогона на Pixel (`docs/call-bugs-needing-you.md`, раздел H). Скрипт молча берёт другое
    устройство, когда заданное пропало, — стоит потребовать явный серийник.
