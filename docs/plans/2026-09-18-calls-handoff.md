...
- Ветка `fix/calls-2026-09`. На `origin` она есть до `dcba454e`; локально сверху ещё 4 коммита, **не запушены**:
  `e059cc1e`, `2c4cb1a7`, `ffeb0a46`, `55d5edc3`.
...
- `docs/manual-verification.md` — все починки, которым нужна проверка на аппарате. Раздел «Ожидают проверки» (14
...
1. **#809 п. 2 — Forta звонит, пока человек уже говорит в Bastyon.** Воспроизведено (`dualb6`, `dualb7`): если страница
   Forta заморожена в фоне, сигнал «ответили на другом устройстве» (`m.call.select_answer`) приходит только через
   синхронизацию JS и не доходит; Forta звонит до таймаута 30 с. Бьёт и по одному аккаунту на двух телефонах.
   - Вариант А: пуш о `m.call.select_answer` — приложение само ставит правило пушей (как `call-hangup-push-rule.ts`
     для отбоя), нативный обработчик сверяет `selected_party_id` со своим `device_id` и гасит звонок, если ответило
     другое устройство. Свой `device_id` нативу нужно передавать при входе (он не секретный).
   - Нужен ли бэкенд: зависит от шлюза пушей. Сейчас в данные FCM он кладёт `room_id`, `event_id`, `sender`,
     `sender_display_name`, `room_name`, `msg_type`, `content_msgtype`, `call_id`, `lifetime` — выборочно. Если
     `selected_party_id` не проходит, попросить бэкенд добавить его (и `party_id`) в данные пуша для событий
     `m.call.*`. Сначала проверить на тестовом аккаунте: временно поставить правило и посмотреть, что пришло.
   - Цена: правило считает каждый принятый звонок непрочитанным на сервере (+1 к счётчику, как у правила для отбоя).
     Список чатов это вычитает (`call-hangup-unread.ts`); бейдж iOS от шлюза и другие клиенты — нет.
...
10. **Пуш и релиз.** Запушить 4 коммита ветки, выпустить сборку, через неделю пересчитать отчёты о звонках
