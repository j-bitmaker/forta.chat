...
2. `npx vue-tsc --noEmit` — проверка типов
3. `npm run test` — тесты
4. Kotlin-тесты при правках в `android/`: `cd android && ./gradlew :app:testSideloadDebugUnitTest --rerun-tasks`
   (без `--rerun-tasks` Gradle отдаёт BUILD SUCCESSFUL из кэша, не запустив тесты)
5. Code review агент (`superpowers:code-reviewer`) — архитектурный ревью изменений

Не коммитить, пока все проверки не пройдены.

Отдельного скрипта `lint` в проекте нет — типы закрывает `vue-tsc`.

### Фиксы, которые нельзя подтвердить автоматически

Если корректность правки не доказывается ни тестом, ни типами, ни сборкой —
нужен реальный аппарат, системный экран или обновление поверх старой установки —
добавь запись в `docs/manual-verification.md` **тем же коммитом**. Формат и
примеры — в самом файле. Фикс без такой записи считается незавершённым, если
проверить его автоматически было нельзя.
